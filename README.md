...
- `GET /admin/actions.html`：轻量动作管理台页面，聚合动作列表、执行日志、重试队列、指标、业务回链、反馈样本池、评测候选池、反馈质量看板、质量趋势和候选操作审计
