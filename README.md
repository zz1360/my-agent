...
v2.2 开始，项目增加 Dockerfile、生产 compose 样例和启动脚本；HTTP 请求、Agent 问答和诊断链路统一写入 MDC trace 字段；API 错误响应统一包含错误码和 traceId；每次 Agent RAG 检索都会写入 `ai_agent_rag_audit` 并通过审计接口返回。

v2.4 开始，Vue 前端从只读查询升级为权限驱动的运营工作台，支持动作审批执行、知识文档生命周期、质量告警任务和评测发布门禁；前端可通过 Nginx 容器与 Spring Boot 同源部署，CI 会运行 Chromium 端到端测试。

v2.5 开始，系统支持 Spring Security OIDC/BFF 会话认证、可信身份上下文、Vue Query 服务端分页、Element Plus 按需构建、前端遥测与桌面/移动端质量门禁。
