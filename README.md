...
- `GET /chat.html`：轻量聊天页，支持流式回答、历史会话、反馈、引用、工具调用和审计 trace
- `GET /admin/actions.html`：轻量动作管理台页面，聚合动作列表、执行日志、重试队列、指标和业务回链
